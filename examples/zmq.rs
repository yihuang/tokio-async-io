...
async fn a_main() -> io::Result<()> {
    ...
    let client_t = task::spawn_local(run_client(client, endpoint));
    let server_t = task::spawn_local(run_server(server, endpoint));
    client_t.await??;
    server_t.await.unwrap_or(Ok(()))
}

#[tokio::main]
async fn main() {
    let local = task::LocalSet::new();
    local.run_until(a_main()).await.unwrap();
}
